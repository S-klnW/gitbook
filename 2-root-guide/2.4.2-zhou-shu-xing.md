...
**颜色：**`SetTitleColor()`参考[2.5.1](2.4.1-yan-se-shu-xing.md#ji-ben-yan-se)

**大小：**`SetTitleSize()`

**字体：**`SetTextFont()`参考[2.5.](2.4.3-wen-zi-shu-xing.md#zi-ti)3
...
root 可以自动选取轴的范围，如果自动的效果不令人满意，root 也支持手动调整：

对于未分 bin 的数据，例如图表，更改轴范围的方法是使用`SetLimits(vmin, vmax)`函数。E.g.
...
也可以绘制具有所需限制的框架，然后绘制图形。此方法允许指定比原始范围更大的轴范围。
...
直方图的轴范围可以使用`SetRange( , )`来设置，参数是 bin 编号的左右位置：
...
使用“用户坐标”而不是 bin 编号来设置范围更方便。而且允许用户直接在直方图对象上设置轴范围，而无需使用中间`Get[XYZ]axis()`。E.g.
...
`SetMinimum()`也会自动执行调用`SetMaximum()`
