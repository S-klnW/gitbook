...
ROOT 有<mark style="color:purple;">**命令行模式、脚本模式和图形界面**</mark>**&#x20;3 种模式。**&#x4F46;是经常使用图形界面会使用户很快遗忘各种类和成员函数的名称和使用方法，这在使用脚本模式处理大量数据时是非常不利的。
...
* **全局变量，**&#x4EE5; `g` 开头，后跟大写字母，如：`gEnv`。通过全局 `gROOT`，可以访问 `TROOT` 的单个实例。\
