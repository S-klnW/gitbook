---
description: >-
  CERN 成立于 1954 年 9 月 29 日，总部位于瑞士日内瓦的法瑞边境上，拥有世界上最大的粒子物理学实验室，也是万维网的发源地。CERN
  现在已经聘用大约三千名的全职员工。并有来自 80 个国籍的大约 6500 位科学家和工程师，代表 500
  余所大学机构，在此进行试验。这大约占了世界上的粒子物理学圈子的一半。
---

# 2.0 安装与使用

由 CERN 开发的科学分析 PB 级数据的强大工具 ROOT 已经成为最著名的开源数据分析框架之一，支持 C 与 Python 语言，用户可通过 [ROOT 官网](https://root.cern.ch)和[查阅手册](https://files.gitbook.com/v0/b/gitbook-x-prod.appspot.com/o/spaces%2Fpf9IsLOaNy0SNtz1zmqo%2Fuploads%2FBytp5axuHdox0qO9IUwt%2FROOT%20User's%20Guide%20Ver.2018.pdf?alt=media\&token=b1dc6826-4e55-46c4-b16d-379ba65b59a9)来学习使用。

ROOT 全平台支持，在 Mac 上安装 ROOT 请使用 [homebrew](../0.-linux-commands/3.-mac-terminal.md#shi-yong-homebrew-an-zhuang-mac-ruan-jian)，其他操作系统安装方式参见[官方说明](https://root.cern/install/#macos-package-managers)。

ROOT 有<mark style="color:purple;">**命令行模式、脚本模式和图形界面**</mark> 3 种模式。经常使用图形界面会使用户很快遗忘各种类和成员函数的名称和使用方法，这在使用脚本模式处理大量数据时是非常不利的。
...
安装 ROOT 后，在终端中输入 `root` 就可以打开 ROOT，使用 `.q` 命令退出 ROOT：
...
终端中输入 `root filename` 可以打开 ROOT 格式文件或脚本（宏），并于 ROOT 中查看或运行。

ROOT 中同样可以使用系统命令，只需要在命令前添加 `.!`。e.g.
...
### <mark style="color:green;">函数命名规范</mark> <a href="#naming-conventions" id="naming-conventions"></a>

* **根类**：`T` 开头，如：`TLine`，`TTree`。
* **数据成员**：`f` 开头，如：`fTree`。
* **成员函数**：首大写字母开头，如：`Loop()`。
* **常数**：`k` 开头，如：`kInitialSize`，`kRed`。
* **全局变量**：`g` 开头，后跟大写字母，如：`gEnv`。通过全局 `gROOT`，可以访问 `TROOT` 的单个实例。\
  例如要查找名为 `c1` 的画布，可以输入：`gROOT->GetListOfCanvases()->FindObject("c1")`。
* **获取和设置函数**：首字母大写的 `Get` 和 `Set` 开头，如：`SetLast()`，`GetFirst()`。

### 树 TTree

「树」是 ROOT 中的一个重要概念，它是一种用 Branch 存储和处理大量结构化数据的数据结构。TTree 可以看作是一个表格，其中每一行代表一个事件或数据点，每一列代表一个变量或数据字段。

它是 ROOT 数据分析和可视化的基础，与其他 ROOT 库中的功能和工具相结合，进行数据分析、表达和模拟等操作。在命令行模式中使用 `.ls` 和 Tree 的成员函数查看 Tree 的结构和 Branch：

```
->Show()   # 使用该命令可以指定查看某个编号下的 branch，例如 Show(24)
           # 注意：展示的数据只会保留 6 位有效数字，在 root 中仍以本身的数值存储
->Print()  # 依次打印出每个 branch 的数据
->Scan()   # 每次扫描 25 行 tree 中的数据
           # ->用于通过指针访问对象的成员，这三个函数都是 Tree 这个类的成员函数
```
...
<summary>查看 Branch 的几种方法</summary>
...
<summary>canvas V.S. pad</summary>
...
有时脚本 `filename` 的运行时间可能会长达几个小时或者几天，为避免终端断连后程序退出，ROOT 支持设置后台运行。e.g. 后台运行并保存输出日志 `log.out`：
...
# 设置 r++=g++ `root-config --cflags --libs` 后, 可使用 r++ sample.cxx -o simple
```
...
* ROOT 程序实例地址：`ls $ROOTSYS/tutorials/`
