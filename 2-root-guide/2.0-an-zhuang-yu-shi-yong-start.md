...
ROOT是CERN开发的科学分析PB级数据的强大工具，用户可通过[ROOT官网](https://root.cern.ch)和[查阅手册](2.0-an-zhuang-yu-shi-yong-start.md#root-jiao-cheng-ji-he)来学习。
...
ROOT全平台可用，在MacOS安装root使用 [homebrew](../0.-linux-commands/3.-mac-terminal.md#shi-yong-homebrew-an-zhuang-mac-ruan-jian)，其他操作系统的安装方式参考[官方说明](https://root.cern/install/#macos-package-managers)：
...
终端中输入`root <filename>`打 root格式文件或运行root脚本，并于root中查看或运行：
