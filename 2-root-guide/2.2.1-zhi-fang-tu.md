...
一维直方图（1D Histogram）是一种对数据分布情况的图形表示方式，适合用来表现数量上的差异。一维直方图的创建方法有很多，一下写法等价：

```cpp
TH1 *h1 = new TH1I("histogram name", "grapg title; x title; unit title", nbins, xmin, xmax)
TH1* h1 = new TH1I("histogram name", "grapg title; x title; unit title", nbins, xmin, xmax)
TH1 *h1 = new TH1I("histogram name", "grapg title; x title; unit title", , , )
TH1 *h1 = new TH1I("histogram name", "grapg title", , , )
TH1 *h1 = new TH1I("histogram name", "grapg title", nbins, xmin, xmax)
auto *h1 = new TH1F
```
...
histogram 也可以通过数据值/坐标来绘制 _**E.g.**_
...
这种方式绘制的 histogram 其实并不能反映bin内的真实值，所以这种方法在拟合的时候往往会造成错误的结果，所以这里提供了一种使用`SetBranchAddress`的方法，通过数据点绘制正确反映 bin 值的histogram：
