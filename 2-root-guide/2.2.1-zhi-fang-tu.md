...
直方图（histogram）与图表（graph）的区别在于使用 bin 分割数据。bin 的原意是垃圾箱或小箱子，在 histogram 中用竖状的 bar 反映数据，这些存放数据的 bar，称为 bin，也就是将整个数据范围划分为一系列统计区间。bin 的宽度取决于轴上 bin 的数量。
...
```cpp
TH1 *h1 = new TH1I("h1", "grapg title; x title; unit title", nbins, xmin, xmax)
TH1* h1 = new TH1I("h1", "grapg title; x title; unit title", nbins, xmin, xmax)
