...
当使用选项“SAME”绘制直方图时，不会绘制统计框。要强制绘制统计框，必须使用选项“SAMES”。如果新的统计框隐藏了前一个统计框，则可以使用以下行更改其位置（“`h`”是指向直方图的指针）：
