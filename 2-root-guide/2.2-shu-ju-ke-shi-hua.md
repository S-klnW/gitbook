...
`TGraph`的创建方法有很多，但本质是一样的，下面的几写法等价。都创建了一个名字是`name`，标题是`title`，有`n`个坐标为`x,y`的点：
...
使用`TGraph::Draw()`绘制图形，且有多种[绘图选项](2.2-shu-ju-ke-shi-hua.md#hui-tu-xuan-xiang)可选：
...
{% content-ref url="2.4.0-shu-xing-she-zhi.md" %}
[2.4.0-shu-xing-she-zhi.md](2.4.0-shu-xing-she-zhi.md)
{% endcontent-ref %}
...
当需要分别使用多张 graph 来表示数据变化或者相互对比时，常用多图绘制的方法。

### 分别绘制

使用`Divide()`函数将 Canvas 分割为不同的 Pad，例如将名为 c1 的 canvas 分为 2 列 3 行写作：`c1->Divide(2,3)`，然后使用`c1->cd()`分别控制不同的 Pad，pad 的名称从左向右，从上至下分别是1，2，……，例如可以在 pad1 上绘制 histogram，在 pad2 上绘制 graph。

### 共轴画法

最简单的共轴画法是在`Divide()`函数中添加参数，例如`c1->Divide(2,3,0)`，但当要进行一些美化工作的时候，就会发现它们的标题大小，marker 大小不再能通过统一的数值控制，这非常影响美观。
...
TMultiGraph允许[将](https://root.cern/doc/master/classTMultiGraph.html)一组图作为单个实体进行操作。特别是，绘制时，X 轴和 Y 轴范围会自动计算，因此所有图形都将可见。

[`TMultiGraph::Add`](https://root.cern/doc/master/classTMultiGraph.html#a2e3c69dd70582da81cf1e32414cc5e2d)应该用于将新图表添加到列表中。
