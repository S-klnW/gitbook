...
创建`TGraph`的写法有很多，但本质是一样的，下面的写法是等价的，都创建了一个名字是`name`，标题是`title`，有`n`个坐标为`x,y`的点：
...
数据的保存格式有很多，常见的是文本格式、数组格式和 root 格式等，都使用`TGraph`类绘制：
...
<summary>E.g.  绘制三个函数</summary>

```c
void exclusion(){
    
    TCanvas *c1 = new TCanvas( "c1","Exclusion graph example",700,500 );
    c1->SetGrid();
...
<figure><img src="../.gitbook/assets/截屏2024-04-08 20.19.47.png" alt="" width="375"><figcaption><p>绘制三个函数</p></figcaption></figure>
...
root 格式支持保存数据，也支持保存分析后的图片，而且图片保存为 root 格式后还可以反复打开并进行进一步的美化工作。
