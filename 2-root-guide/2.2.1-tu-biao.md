...
数据可视化的方法有很多，最基本的是使用由两个或三个数组组成的坐标表示数据的大小或位置，称作图表（Graph）。这与直方图（Histogram）的表达方式不同。

ROOT 提供了许多图形类，如 `TGraph` ；`TGraphErrors` 具有对称误差线的图表；`TGraph2D` 由三个数组 `X、Y、Z` 组成的图表等。
...
```c
TGraph *gr = new TGraph("name", "title", number, x, y)    // 创建一个名字是 name，标题是 title，有 n 个坐标为 x,y 的点
TGraph* gr = new TGraph("name", "title", number, x, y)    // 创建一个名字是 name，标题是 title，有 n 个坐标为 x,y 的点
TGraph *gr = new TGraph(number, x, y)    // 创建一个有 n 个坐标为 x,y 的点
auto gr = new TGraph(number, x, y)    // 创建一个有 n 个坐标为 x,y 的点
auto gr = new TGraph()        // 使用 AddPoint() 函数添加坐标点
...
* `double *px = gr->GetX()`；将 `gr1` 图像中点数据以数组形式输出。
...
使用以下选项绘制图表，在 `Draw()` 命令中它们<mark style="color:$warning;">不区分大小写</mark>，也可不间隔的使用多个参数，如 `Draw("APL")` 和 `Draw("A p l")`，均表示绘制包涵坐标轴和坐标点的图，并用直线相连：

<table><thead><tr><th width="111">Option</th><th>Description</th></tr></thead><tbody><tr><td><code>"A"</code></td><td><mark style="color:green;"><strong>绘制出坐标轴</strong></mark></td></tr><tr><td><code>"I"</code></td><td>Combine with option 'A' it draws invisible axis</td></tr><tr><td><code>"L"</code></td><td>在点之间使用直线相连，最终呈现的效果是折线</td></tr><tr><td><code>"F"</code></td><td>绘制填充区域（ <code>CF</code> 绘制平滑填充区域）</td></tr><tr><td><code>"C"</code></td><td><mark style="color:green;"><strong>绘制平滑曲线</strong></mark></td></tr><tr><td><code>"*"</code></td><td>每数据点以星号显示</td></tr><tr><td><code>"P"</code></td><td>绘制出每个数据点</td></tr><tr><td><code>"B"</code></td><td>绘制柱状图/条形图/直方图</td></tr><tr><td><code>"1"</code></td><td>当使用柱状图 B 时，使柱状图从底部开始，否则从 0 开始</td></tr><tr><td><code>"X+"</code></td><td>横轴在上</td></tr><tr><td><code>"Y+"</code></td><td>纵轴在右</td></tr><tr><td><code>"PFC"</code></td><td><mark style="color:$warning;">调色板填充颜色：图表的填充颜色取自当前调色板中</mark></td></tr><tr><td><code>"PLC"</code></td><td><mark style="color:$warning;">调色板线条颜色：图形的线条颜色取自当前调色板中</mark></td></tr><tr><td><code>"PMC"</code></td><td><mark style="color:$warning;">调色板标记颜色：图表的标记颜色取自当前调色板中</mark></td></tr><tr><td><code>"RX"</code></td><td><mark style="color:$warning;">X轴刻度反转</mark></td></tr><tr><td><code>"RY"</code></td><td>Y轴刻度反转</td></tr><tr><td><code>"HIST"</code></td><td>以histogram格式绘制/亦或称作阶梯图</td></tr></tbody></table>

<details>

<summary>e.g. 横轴在上，纵轴反转和自动取色</summary>
...
<figure><img src="../.gitbook/assets/rlx+palecolor.svg" alt="" width="425"><figcaption></figcaption></figure>

### 成果保存

ROOT 格式支持保存数据，也支持保存分析图片，而且图片保存为 ROOT 格式后还可以反复打开并进行进一步的美化工作，十分便捷。

`TGraph::Save()`：将图表保存为一个对象文件，如 `.csv`、`.tsv` 或 `.txt` 和**其他的图像格式**。

<mark style="color:$success;">`TGraph::SaveAs()`</mark><mark style="color:$success;">：另存为，将图表保存任意图像格式。</mark>

`TPad::Print()` or `TCanvas::Print()`：打印画布，并可指定格式。
...
`TFile::Write()`：将 `TTree` 对象保存到 ROOT 文件中。e.g.
...
<figure><img src="../.gitbook/assets/graph2d.png" alt="" width="375"><figcaption></figcaption></figure>

### 绘图选项

<table><thead><tr><th width="114.67578125">参数</th><th>描述</th></tr></thead><tbody><tr><td><code>"TRI"</code></td><td>填充 Delaunay 三角形，绘制表面和边。</td></tr><tr><td><code>"TRIW"</code></td><td>以线框模式绘制 Delaunay 三角形。</td></tr><tr><td><code>"TRI1"</code></td><td>使用颜色级别填充 Delaunay 三角形，绘制边。</td></tr><tr><td><code>"TRI2"</code></td><td>使用颜色级别填充 Delaunay 三角形，不绘制边。</td></tr><tr><td><code>"P"</code></td><td>在每个顶点绘制标记。</td></tr><tr><td><code>"P0"</code></td><td>在每个顶点绘制圆形标记，背景为白色。</td></tr><tr><td><code>"PCOL"</code></td><td>在每个顶点绘制标记，颜色根据Z值决定。</td></tr><tr><td><code>"LINE"</code></td><td>绘制 3D 折线。</td></tr><tr><td><code>"CONT5"</code></td><td>使用 Delaunay 三角形绘制等高线图。</td></tr></tbody></table>
...
<table><thead><tr><th width="155">Option</th><th>Description</th></tr></thead><tbody><tr><td><code>"O"</code></td><td>极坐标标签与极坐标图半径正交绘制。</td></tr><tr><td><code>"P"</code></td><td>在每个点位置绘制多标记。</td></tr><tr><td><code>"E"</code></td><td>绘出误差棒</td></tr><tr><td><code>"F"</code></td><td>绘制填充区域（闭合多边形）</td></tr><tr><td><code>"A"</code></td><td>即使极坐标图已经存在，也强制重绘轴</td></tr><tr><td><code>"N"</code></td><td>禁止显示极坐标标签</td></tr></tbody></table>
...
ROOT 支持在一张画布上绘制多幅图表或者直方图。

### 独立绘制

使用 `TCanvas::Divide()` 函数将画布割为不同的画板，例如将名为 `c1` 的画布分为 2 列 3 行写作：`c1->Divide(2,3)`，然后使用 `c1->cd()` 分别控制不同的画板，画板的名称从左向右，从上至下分别是 1，2，……，例如可以在第一个画板 `pad1` 上绘制直方图，在第二个画板 `pad2` 上绘制图表。

### 共轴绘制

最简单的共轴画法是在 `TCanvas::Divide()` 函数中添加参数，例如 `c1->Divide(2,3,0)`，但当要进行一些美化工作的时候，就会发现它们的标题大小、记号（TMarker）大小不再能通过统一的数值控制，这非常影响美观。所以这里有一个稍微复杂的解决方法，能够使它们的大小保持一致，仅供参考：
...
* 将多个图表绘制在一个坐标系上，只需要在 `TGraph::Draw()` 函数中使用 `same` 参数即可。
* `TMultiGraph::TMultiGraph()`，函数支持将一组图作为单个实体进行操作。且绘制时 X轴 和 Y轴 范围会自动计算，因此所有图形都将可见。[`TMultiGraph::Add()`](https://root.cern/doc/master/classTMultiGraph.html#a2e3c69dd70582da81cf1e32414cc5e2d) 用于将新图表添加到列表中。e.g.
...
<mark style="color:green;">使用</mark> <mark style="color:green;">`c1.SetFrameBorderMode(0);`</mark> <mark style="color:green;">函数让边界消失。</mark>
...
在 ROOT 中，`TLegend` 类用于在绘图中添加图例（Legend），以帮助解释图形中的不同元素（如曲线、直方图、标记等）。`TLegend` 可以灵活地设置图例的位置、大小、内容、样式等。
...
* [TGaxis Class Reference](https://root.cern/doc/master/classTGaxis.html)
* [TGraphPainter Class Reference](https://root.cern/doc/master/classTGraphPainter.html)
* [THistPainter Class Reference](https://root.cern.ch/doc/master/classTHistPainter.html)
* [TLegend Class Reference](https://root.cern.ch/doc/master/classTLegend.html)
