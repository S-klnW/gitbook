...
使用 `TCanvas::Divide()` 函数将画布割为不同的画板，例如将名为 `c1` 的画布分为 2 列 3 行写作：`c1->Divide(2,3)`，然后使用 `c1->cd()` 分别控制不同的画板，画板的名称从左向右，从上至下分别是1，2，……，例如可以在第一个画板 `pad1` 上绘制直方图，在第二个画板 `pad2` 上绘制图表。
