...
【注】计算机视觉中，颜色渲染利用了蒙版叠加（Alpha Blending）的技术，将 35% 的前景色与 65% 的背景色按比例混合，从而呈现出半透明的效果。因此 100% 是完全不透明，0% 是完全透明。
...
如果调色板的色彩呈现你不满意，可以尝试调色板反置 `TColor::InvertPalette` e.g.
...
或者使用 `TH1::SetContour()` 或`TStyle::SetNumberContours()` 进行更改。 这个数字越高，单元格之间的颜色变化就越平滑。用于绘制单元格的默认颜色级别数为 `20`。

【注】如果直方图的最小值和最大值相同（平面直方图），则无法绘制。 要绘制平面直方图，只需设置与 bin 内容不同的直方图最小值 `TH1::SetMinimum()`。

`TStyle` 中的调色板可以通过 `gStyle->SetPalette()` 进行修改。
...
* `SetFillStyle(4000-4100)` 意为 0% 至 100% 填充不透明度。
...
* [PHOTOKIT｜创建和谐的渐变调色板](https://photokit.com/colors/color-gradient/?lang=zh)
* [uigradients｜无极渐变方案](https://uigradients.com/#WitchingHour)
* [Package - RColorBrewer ｜emilhvitfeldt.github.io｜一些适合文章发表的配色方案](https://emilhvitfeldt.github.io/r-color-palettes/discrete.html#category=Package%20-%20RColorBrewer)
* [pxlab｜线性渐变方案](https://pxlab.cn/)
...
* [为什么画布保存为 PDF 会在像素块边缘存在一个单位像素的白线？](https://root-forum.cern.ch/t/colored-2d-histograms-in-pdf-output/13822/37)
