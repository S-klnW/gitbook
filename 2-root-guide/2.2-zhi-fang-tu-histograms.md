...
bin的原意是垃圾箱或小箱子，histogram中用竖状的bar存放数据，bar的高度反映数据的多少，我们将存放数据的bar，称为bin。bin的宽度取决于轴上bin的数量。

当canvas和轴发生变化时，bin宽（形式上而非数值上）也会发生变化。如`SetLogx`，表示将x轴设置为对数坐标的形式，此时bin宽（形式上）会随logx而变化，若要求bin宽（形式上）不随logx变化，可以参考[这里](https://www.bilibili.com/read/cv6640287/)。
