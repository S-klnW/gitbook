...
使用 `TFile` 中的 `Open()` 函数来创建或打开 ROOT 文件。
...
<summary>e.g. 向一个 ROOT 文件中写入数据</summary>
...
<summary>e.g. 从 ROOT 文件中读取数据到文本</summary>
...
<summary>e.g. 利用动态数组 c 绘制</summary>
...
用户可以将任何对象（例如画布或直方图）保存到 ROOT 文件中。甚至可以存储用户自己创造的类型。

`TFile` 源自 `TDirectory`；使用 `TDirectory::WriteObject()` 将对象写入 ROOT 文件。e.g.
...
## ROOT 命令行工具 <a href="#root-command-line-tools" id="root-command-line-tools"></a>
...
* `rootbrowse`：直接打开带有 ROOT 文件内容的 [`TBrowser`](https://root.cern/doc/master/classTBrowser.html)。
...
使用 `-h` 选项获取有关特定 ROOT 命令行工具可用选项的更多信息。

在系统提示符上，您可以使用 ROOT 命令行工具 `rootls` 来列出 ROOT 文件的内容。e.g.
...
当需要一个基础的计算功能时，可以使用 ROOT。当然也可以使用 [**exper命令**](../0.-linux-commands/1.4-linux-ming-ling-fu-lu.md#expr-ming-ling-ming-ling-xing-ji-shu-qi)。e.g.
...
`TGraph::Eval(x, method, option)` ​函数用于对离散数列进行插值计算，返回指定位置 `x` 处的插值结果。若 `x` 超出当前图形的横坐标范围，则执行**线性外推**。
...
<summary>e.g. Eval 函数插值计算实例</summary>
...
`TH1::Interpolate(x)、TH2::Interpolate(x, y)、TH3::Interpolate(x, y, z)` 对直方图进行线性插值。可以在任意坐标点取值，而不仅限于 [Bin](2.2.2-zhi-fang-tu.md#zhi-fang-tu-de-gai-nian) 中心。

<table data-full-width="false"><thead><tr><th width="226">拟合参数 method</th><th>含义</th></tr></thead><tbody><tr><td><code>kLINEAR</code></td><td>线性插值</td></tr><tr><td><code>kPOLYNOMIAL</code></td><td>多项式插值，用于少量点，因为会引入较大的振荡</td></tr><tr><td><code>kCSPLINE</code></td><td>具有自然边界条件的三次样条</td></tr><tr><td><code>kCSPLINE_PERIODIC</code></td><td>具有周期边界条件的三次样条</td></tr><tr><td><code>kAKIMA</code></td><td>具有自然边界条件的 Akima 样条（至少需要 5 个点）</td></tr><tr><td><code>kAKIMA_PERIODIC</code></td><td>具有周期边界的 Akima 样条（至少需要 5 个点）</td></tr></tbody></table>

**数据平滑与重采样**

`TGraph::Approx()` 函数用于对离散数据进行插值重采样。通过在已有数据点之间生成新的中间点，使曲线更平滑或密度更高。常用于原始数据点稀疏的情况。
...
* `TGraph::Integral(first, last)`\
  此函数会计算 `TGraph` 所包围的多边形的面积。**按点顺序**构成的闭合多边形的有向面积（点的顺序至关重要），另请注意，线段不应相交。如果 `last=-1`（默认）`last` 设置为最后一个点。如果 `(first < 0)`，则采用第一个点 `(0)`。
* `TH1::Integral(binx1, binx2, option="")`\
  默认情况下，积分计算为范围内 Bin 内容的总和。如果指定了选项 `width`，则积分是 Bin 内容乘以 `x` 中的 Bin 宽度之和。

### 条件判选

数据处理过程往往需要为数据添加一些限制条件，使结果达到希望呈现的样子，在脚本中，按照 C 语言规范正常添加即可，在命令行模式中添加限制条件，写法如下：
...
多个限制条件之间使用 `&&` 符号相连，或 `||`，也可以使用 `TCut` 类合并。
...
gRandom->Rannor (Double_t &a, Double_t &b)    // 返回两个符合 Gaus(0, 1) 分布的数
...
* [TRandom Class Reference](https://root.cern/doc/master/classTRandom.html)
