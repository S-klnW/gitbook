...
<mark style="color:red;">注意：拟合操作应在最后进行，如果宏中的标记属性设置在拟合之后，可能会导致拟合失效</mark>



## 误差棒

误差棒（error bar）在统计图表中起到了传达数据不确定性和可靠性的作用，帮助读者更好地理解数据并做出准确的解释和推断。任何测量都会有误差，使用 `TGraphAsymmErrors()` 函数绘制误差棒，用户甚至可以绘制倾斜的误差棒。
...
* `SetFillColor()` 设置误差棒的填充颜色
* `SetFillStyle()` 设置误差棒的填充风格
* `SetLineColor()` 设置误差棒的颜色
...
选项 `"4"` 与选项 `"3"` 相似，只是表带是平滑的。 如上图所示，<mark style="color:red;">**应谨慎使用此选项，因为平滑算法可能会显示一些（巨大的）“弹跳”效果。 在某些情况下，它看起来比选项**</mark><mark style="color:red;">**&#x20;**</mark><mark style="color:red;">**`"3"`**</mark><mark style="color:red;">**&#x20;**</mark><mark style="color:red;">**更好（因为它很光滑），但它可能会具有误导性。svg 和其他保存格式在显示上会略有差异，这里只做示意。论坛上关于弹跳的提问与解答：**</mark>[https://root-forum.cern.ch/t/confidence-band-on-tf1-fit/34514](https://root-forum.cern.ch/t/confidence-band-on-tf1-fit/34514)



## 多重误差

在粒子物理实验中，一个测量结果通常包含多种不同来源的误差，最典型的便是统计误差（Statistical Error）和系统误差（Systematic Error）。

`TMultiErrors` 是一个类，用于处理多个测量值的误差。它是 ROOT 数据分析框架中的一部分，用于处理数据的不确定性和误差。
...
ROOT 内置了数个已经定义好的标准数学函数，可以直接一键调用。

```c--
hist.Fit("gaus");
```

<table data-header-hidden><thead><tr><th width="112">字符串</th><th width="201">对应数学模型</th><th>适用物理场景</th></tr></thead><tbody><tr><td><code>"gaus"</code></td><td>高斯分布</td><td>测量误差分析、能谱中的光电峰/信号峰拟合</td></tr><tr><td><code>"expo"</code></td><td>指数衰减</td><td>放射性核素衰减、宇宙线谬子寿命测量、本底估计</td></tr><tr><td><code>"polN"</code></td><td>N 阶多项式（如 <code>"pol2"</code>、<code>"pol3"</code>）</td><td>复杂的连续谱本底（Background）平滑拟合</td></tr><tr><td><code>"landau"</code></td><td>朗道分布</td><td>带电粒子穿过薄探测器（如硅微条、气体正比计数器）的电离能量损失拟合</td></tr></tbody></table>

#### 限制拟合区间

```
hist.Fit("函数名", "选项字符串", "R(可选限制区间)", X小值, X大值);
```

```
// 只在 x 轴的 1.2 到 3.5 之间进行高斯拟合
hist.Fit("gaus", "R", "", 1.2, 3.5);
```

拟合完成后，用户可以通过 `TF1* myfunc = hist.GetFunction("gaus");` 随时提取出拟合得到的均值（Mean）、标准差（Sigma）以及对应的 $$\chi^2/\text{ndf}$$ 检验值。
...
如果某个参数有明确的物理含义（例如高斯分布的宽度 $$σ$$ 必须为正数，或者能谱峰的位置已知在某个区间内），应该对其进行限制。

```c
// 语法：f1->SetParLimits(参数索引, 最小值, 最大值);
// 限制参数 2 的取值只能在 0.1 到 5.0 之间
f1->SetParLimits(2, 0.1, 5.0); 
```

在多成分拟合中，如果已经通过其他实验独立测量出了某个参数（例如本底的斜率），可以将该参数牢牢固定，不让它参与当前的拟合迭代，从而提高其他参数的拟合精度。

```c
// 将参数 1 的值死死固定在 3.14，拟合过程中该参数保持不变
f1->FixParameter(1, 3.14);

// 替代方案：将 Limits 的最大值和最小值设为相等，效果等同于固定参数
f1->SetParLimits(1, 3.14, 3.14);
```
...
拟合后，拟合得到的最优参数和卡方检验值值就已经存储在 `f1` 对象中了。用户可以通过以下命令将其提取到 C++ 变量中进行后续计算：

`TF1::GetChisquare()`：获取拟合的卡方值 $$\chi^2$$ 。

`TF1::GetNDF()`：获取拟合的自由度。<mark style="color:$warning;">优秀的物理拟合，其卡方值（</mark>$$\chi^2/\text{ndf}$$<mark style="color:$warning;">）应该非常接近 1。</mark>

`TF1::GetParameter(int i)`：获取第 `i` 个参数的最优拟合值（返回 `Double_t`）。

`TF1::GetParError(int i)`：获取第 `i` 个参数的标准对称误差。

`TF1::GetParErrorLow(int i),GetParErrorUp(int i)`：获取第 `i` 个参数的下、上限误差。
...
使用不同的标记符号表征不同的实验结果，但横轴范围不一致，那么拟合哪一段的数据最为准确？

使用 `TMultiGraph::TMultiGraph()` 函数，将多个图标合并，既能保留不同标记符号的个性，也能让所有数据点参与拟合。

#### 分别拟合

<details>

<summary>e.g. 多条拟合线绘制在一张图表上</summary>
...
ROOT 默认会在图像右上角渲染一个统计框。面板的内容、位置以及参数名称支持自定义。

手动设置直方图拟合框：`TStyle::SetOptFit(pcev)`。
...
常用搭配：
gStyle->SetOptFit(0)    // 彻底关闭拟合信息面板，保持图像纯净。
gStyle->SetOptFit(1)    // 等价于简写，默认开启基本信息。
gStyle->SetOptFit(1111)    // 全信息开启（依次显示：p值、chi^2/ndf、误差、参数名与值。
gStyle->SetOptFit(1011)    // 显示卡方、误差、参数，但隐藏 p值。
```

### 拟合面板位置

适用于打开多个拟合面板，或避免面板对图像的遮挡（设置[透明](2.3.1-tian-chong-shu-xing.md#tou-ming-shi-tu)亦可）。

`TPave::SetX1 (x1)` 设置拟合框的左下角的值为 `x1`。

`TPave::SetX1NDC(newx1)` 设置拟合框相对位置，取值范围 `(0,1)`。NDC 是 Normalized Coordinate 的缩写，指的是归一化坐标体系。

`TPave::SetX2NDC(newx2); TPave::SetY1NDC(newx2);TPave::SetY2NDC(newx2);` 同理。

### 拟合参数名

高斯分布默认显示为 `Constant`、`Mean`、`Sigma`；多项式默认显示为 `p0`、`p1`、`p2`。在面向特定物理主题时，修改参数名称可以让你的拟合结果更加清晰（如 $$A$$、$$μ$$、$$σ$$ 等）。

`TF1::SetParNames("parname", "", "parname")` 最多支持 `10` 位的参数名称重制，空字符串会被跳过。
...
* 如果填入直方图的数据是普通的「计数」（没有带额外权重），ROOT 会自动把误差定为计数量的开方；如果数据具有权重，就需要增加 `h->Sumw2();`，否则画出来的误差棒就是错的。
* 如果直方图里数据很少，甚至有很多没有计数的空 Bin，应该在拟合参数里加上 `"L"`（对数似然法），这样算出来的物理结果和误差才最精准。
* 获取直方图某一个特定 Bin 的误差值：

```c
// 1. 获取第 10 个 Bin 的数据计数（Bin 的高度）
Double_t content = h->GetBinContent(10);

// 2. 获取第 10 个 Bin 的统计误差（误差棒的上下半边长）
Double_t error   = h->GetBinError(10);

// 3. 在终端打印出来，常用于检查数据或手动计算显著性
cout << "Bin 10 的结果是: " << content << " +/- " << error << endl;
```
...
* [TMultiGraph Class Reference](https://root.cern.ch/doc/master/classTMultiGraph.html)
