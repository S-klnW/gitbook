...
`TGraphMultiErrors` 的工作方式与 `TGraphAsymmErrors` 基本相同。 它有可能定义多个 `y-Errors` 的类型/维度。 如果你想同时绘制统计和系统错误，这非常有用。
...
修改参数名称可以让你的拟合结果更加清晰，`TF1::SetParNames("parname", "", "parname")` 最多支持 `10` 位的参数名称重制，空字符串会被跳过。
