...
**【注】**&#x767B;陆时，可能出现如下错误：免密登陆服务器提示 `/usr/bin/xauth: timeout in locking authority file /afs/ihep.ac.cn/users/y/yyguo/.Xauthority`，且无法正常启动 X11 服务。请删除 `.Xauthority` 之后重新登陆，亦或执行 `kinit && aklog`。这个问题出现的原因似乎和免密登陆的认证有关，具体原因还在排查中。



## 在 Mac 或移动硬盘上安装其他系统

请参考哔哩哔哩博主「黄杨ME」的[视频投稿](https://www.bilibili.com/video/BV1EX4y1Y7wZ/?spm_id_from=333.999.0.0\&vd_source=745889680230532a09ad7b768ffb84d1)。
...
这个过程会因 macOS 版本而有所不同。其他版本命令参考[这里](https://www.sysgeek.cn/flush-dns-cache/)。
...
点击安装软件 [FixTim](https://github.com/Lakr233/FixTim/)。
...
参考「谷中望月」的这篇[博客](https://kukmoon.github.io/75f44950511b/)。
...
安装某些软件后，一些文件图标会被强制修改，如果需要恢复默认样式，使用下面命令恢复：
...
## 从音乐软件中同步歌单至 Apple music

在线工具 [Soundiiz](https://soundiiz.com/)。
