...
* **Command-I**：查看文件信息；**Option-Command-W**：关闭打开的所有信息窗口（<mark style="color:green;">这在不小心打开多个文件信息窗口时非常有用</mark>）；**Option-Command-I：**&#x6587;件信息窗口汇总打开。
...
#### 在 Mac terminal 中使用 ssh 连接服务器。e.g.
...
参见 bilibili up主黄杨ME的[视频](https://www.bilibili.com/video/BV1EX4y1Y7wZ/?spm_id_from=333.999.0.0\&vd_source=745889680230532a09ad7b768ffb84d1)。
...
{% hint style="info" %}
**DNS（域名系统）**&#x7F13;存是记录应用程序（例如 Web 浏览器）向 DNS 服务器发出的所有查询的**临时数据库**。
...
在`.vimrc`中启动：`set clipboard=unnamed`选项。
...
安装某系软件（WPS）后，一些文件图标会被强制修改样式，如果你喜欢默认图标样式，可以使用以下命令恢复：
