...
macOS 是运行在 Mac 上的“类 UNIX“操作系统，本质上和 Linux 都属于 UNIX 家族。
...
`Homebrew` 是一款包管理工具，支持 `macOS` 和 `Linux` 系统。更多了解参见[官网](https://brew.sh)及 [homebrew 论坛](https://brew.idayer.com)。
...
这里，可以使用[#pei-zhi-bie-ming](1.1-xi-tong-guan-li-lei-ming-ling.md#pei-zhi-bie-ming "mention")来避免每次繁琐的输入这些复杂的指令。
...
这个过程会因 macOS 版本而有所不同。其他版本命令参考这篇[博文](https://www.sysgeek.cn/flush-dns-cache/)。
...
在 `.vimrc` 中添加命令 `set clipboard=unnamed`。
...
1. 下载 [iSH Shell](https://apps.apple.com/us/app/ish-shell/id1436902243)；
2. 使用命令 `mount -t ios ./mnt` 将 iPhone 挂载到 `./mnt` 目录；
3. 使用命令 `cd /mnt` 进入挂载的目录；
4. 使用命令 `ls -a` 列出挂载目录下的所有文件和文件夹。

**参考**：[\[Mobile\] IOS : App to work with hidden folder](https://forum.obsidian.md/t/mobile-ios-app-to-work-with-hidden-folder/25741)
...
```shell
MacBook-Pro-3:~ zhangzh$ security find-generic-password -wa wifi名称
```
...
1. 新建自动操作；
2. 点击「新建文稿」，选择「快速操作」；
3. 工作流程选择「文件及文件夹」，位于「访达」，左侧选择「运行 shell 脚本」；
4.  双击添加：
    ...
    ```
5. `command-S` 保存，并在任意需要压缩的文件上右击，选择在「快速操作」中使用该自动操作。
...
1. 前往 `cd /Library/Audio/Plug-Ins/HAL/`，`rm -rf` 删除对应驱动。
2. 输入 `sudo killall coreaudiod` 重启服务。



## 在访达中挂载服务器

<div align="left"><figure><img src="../.gitbook/assets/MacMiniM4.png" alt="" width="188"><figcaption></figcaption></figure></div>
...
7.  挂载服务器目录：
    ...
    ```bash
    mkdir ~/remote
    sshfs -p 22 zhangzh@172.25.73.13:/ ~/remote
    ```
8. 完成。

#### **若不再需要通过访达挂载远程目录，请按以下步骤完整还原系统**
...
2.  卸载 SSHFS：
    ...
    ```bash
    sudo rm -f /usr/local/bin/sshfs
    ```
3. 卸载 macFUSE\
   重新下载最新版 macFUSE 安装镜像，挂载 dmg，运行其中 **Extras 文件夹内的 Uninstaller**，即可彻底移除 macFUSE 全部组件。
4. 将 Mac 安全策略恢复为「完整安全性」。



## 启用 iMac 专属强调色

<figure><img src="../.gitbook/assets/hero_elnzbc5xeoa6_large_2x.jpg" alt=""><figcaption></figcaption></figure>

iMac 从 M1 开始引入了多种机身配色，并在系统中预留了对应的强调色选项，**但默认隐藏。** 通过修改 macOS `defaults` 配置，可在任意 Mac 上启用这些强调色。
