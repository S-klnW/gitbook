...
从现在开始我们要学会使用 `man` 命令（manual）去阅读命令使用介绍。你可以用 `type 命令` 来判断这个命令到底是可执行文件、shell内置命令还是别名，`type`的使用方法参考[附录](1.2.-wen-jian-guan-li-lei-ming-ling.md#type-ming-ling-xian-shi-zhi-ding-ming-ling-de-lei-xing)。**注意：**
