...
Linux 本身是一个内核，只有加上其他一些组件之后才是我们熟悉的真正意义上的系统。后来，随着 Linux 的流行，人们逐渐默认了 Linux 就代指系统，你也可以严格的称其为GNU/Linux系统。

在计算机发展初期，计算机非常大（以吨为单位）。因此，计算机一般都是放到一个单独的房间中，人们在另一个地方通过一定的手段（设备）去操控计算机，操控计算机的这个设备就被称为终端，也叫做终端机。终端其实就是一种输入输出设备，相对于计算机主机而言属于外设，本身并不提供运算处理功能。&#x20;
...
进入终端之后，你会看到一个`[]$`的结构，后面跟着闪烁或常量的光标，这就是一个shell界面，我们通过shell和计算机进行交互。本节我将总结并介绍一些基础的命令。
