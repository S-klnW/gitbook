...
Linux本身是指一个内核，只有加上其他一些组件之后才是真正意义上的“系统”。后来，随着 Linux的流行，人们逐渐默认了以Linux代指系统，你也可以严格的称其为GNU/Linux系统。

在计算机发展初期，计算机非常大（以吨为单位）。因此，计算机一般都是放到一个单独的房间中，人们在另一个地方通过一定的手段（设备）去操控计算机，操控计算机的这个设备就被称为终端，也叫做终端机。终端其实就是一种输入输出设备，本身并不提供运算处理功能。&#x20;
...
打开个人电脑或的终端之后，你会看到一个`[]$`的结构，后面跟着闪烁或常量的光标，这就是一个shell界面，我们通过shell和计算机进行交互。本节我将总结并介绍一些基础的命令。
...
在root账户下，修改其他用户的密码方式参考[用户管理](1.1.-xi-tong-guan-li-lei-ming-ling.md#yong-hu-guan-li)；找回root用户的密码，参考[这里](http://t.zoukankan.com/canyezhizi-p-13574958.html)。
...
但是每次输入密码登陆未免太过麻烦，在可以信任的计算机上，可以选择免密登陆：
...
#### [跨服务器传输文件](1.2.-wen-jian-guan-li-lei-ming-ling.md#fu-zhi-yi-dong-zhong-ming-ming-shan-chu)



## 开机自启

不同系统的服务启动指令略有差异，在Centos中，选择自启动的服务时，使用命令`systemctl start git` _**E.g.**_
