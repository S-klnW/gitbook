...
Vim（Vi IMproved）是从 vi 发展出来的一个文本编辑器。代码补全、错误跳转等功能特别丰富，在程序员中被广泛使用。Vim 分为三种模式，分别是**命令模式（Command mode）**，**输入/插入模式（Insert mode）**&#x548C;**底线命令/命令行模式（Last line mode）**。 这三种模式的作用分别是：
