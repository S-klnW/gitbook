...
shell 顾名思义为「壳」，主要是相对于系统内核来说的，可以理解为内核的「壳」。直接接触内核是不被允许并危险的，所以由 shell 负责将用户的指令解释并传给内核执行，可以把 shell 看作是一种命令解释器。常见的 shell 有很多，分为图形界面 shell 和命令行 shell，命令行中最常用的就是 bash shell。
...
Bash 是目前最流行的一种 Shell 实现，用来把你输入的命令翻译成操作系统能执行的东西。

* 在 Bash 中，可以通过 `Tab` 补全命令。
* 使用 `history` 命令查看 `1000` 条命令行历史记录，再输入 `!命令编号` 就可以再次执行该命令。
* 使用 `ctrl-R` 搜索命令行历史记录：进入搜索功能后，输入关键字搜索，重复按下 `ctrl-R` 会向后查找匹配项，按下 `Enter` 键会执行当前匹配的命令，而按下右方向键会将匹配项放入当前行中，不会直接执行，以便做出修改。
...
当程序步骤复杂时，bash 有其局限性，例如需要嵌套语句、分步骤执行等等。

所以要求用户熟悉至少一种基于文本的编辑器，编辑器可以将你的命令按顺序执行，称为脚本。通常 Vim 会是最好的选择。

[Vim](https://www.vim.org/)（Vi iMproved）是从 vi 发展出来的一个文本编辑器。具有代码补全、错误跳转等功能，在程序员中被广泛使用。Vim 分为三种模式，分别是**命令模式（Command mode）**，**输入/插入模式（Insert mode）**&#x548C;**底线命令/命令行模式（Last line mode）**。 这三种模式的作用分别是：
...
按下键盘 `ESC` 键，退出输入模式，切换到命令模式。完成文件的编辑后，在命令行模式中输入 `wq` 存盘退出，也可使用 `Shift-ZZ` 来达到同样的效果。
...
在 Vim 中输入 `:help user-manual` 进入用户手册。也有[手册中文版](https://yianwillis.github.io/vimcdoc/doc/usr_toc.html)可供参考。下面介绍了一些不同模式下的常用命令：
...
<mark style="color:$danger;">定义变量时，</mark><mark style="color:$danger;">**等号**</mark><mark style="color:$danger;">**&#x20;**</mark><mark style="color:$danger;">**`=`**</mark><mark style="color:$danger;">**&#x20;**</mark><mark style="color:$danger;">**两边绝对不能有空格**</mark><mark style="color:$danger;">。</mark>
...
当脚本需要用户输入服务器密码、数据库密钥等隐私信息时，如果直接显示在屏幕上会有安全隐患。使用 `-s` 参数可以关闭终端的回显，输入时屏幕上什么都不会显示，但数据仍能正确存入变量。
...
<mark style="color:$danger;">不是所有的命令都支持管道符</mark><mark style="color:$danger;">，</mark>如 `ls` 类命令 (但可以配合 `xargs` 的命令使用）
