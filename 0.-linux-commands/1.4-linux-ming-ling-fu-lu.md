...
在显示方面，用户可以设定显示格式，格式设定为一个加号后接数个标记，可用的标记列表如下：
...
若是不以加号作为开头，则表示要设定时间，而时间格式为：`MMDDhhmm[[CC]YY][.ss]`，其中：`MM`为月份，`DD` 为日，`hh`为小时，`mm`为分钟，`CC`为年份前两位数字，`YY`为年份后两位数字，`ss`为秒数。当不希望出现无意义的 `0` 时，如 `1999/03/07`。可以在标记中插入 `-` 符号，比如说 `date '+%-H:%-M:%-S'`。

当以 root 身份更改了系统时间之后，输入 `clock -w` 来将系统时间写入 CMOS 中，这样下次重新开机时系统时间才会持续保持最新的正确值。
...
14
[zhangzh@node01 alldata]$ expr substr "this is a test" 3 5  # 统计3-5的字符
is is
...
用于指定增量从首数开始打印数字到尾数，全拼为\<Sequence>**。**&#x5373;产生从某个数到另外一个数之间的所有整数，并且可以对整数的格式、宽度、分割符号进行控制。e.g.
...
任意精度计算器语言，全拼为\<basic alculator>。e.g.
...
e.g. 把 `textfile1` 的文档内容加上行号后输入 `textfile2` 这个文档里：

```
cat -n textfile1 > textfile2
```

e.g. 把 `textfile1` 和 `textfile2` 的文档内容加上行号（空白行不加）之后将内容附加到 `textfile3` 文档里：

```
cat -b textfile1 textfile2 >> textfile3
```

e.g. 清空 `/etc/test.txt` 文档内容：
...
用于将<mark style="color:$warning;">两个文件中</mark>，内容相同的行连接起来。e.g.
...
### <mark style="color:$success;">column命令 文本对齐</mark>

<mark style="color:$success;">将文本格式化成多个对齐、定宽的列或表格</mark>
...
常用于产生大文件或回答 `yes/no` ，不加文本默认输出`y`。e.g.
...
### expand 和 unexpand命令 制表符与空格之间转换

### stat命令 显示文件状态信息

包括文件的类型、权限、大小、所属用户和组、访问时间、修改时间、创建时间等详细信息。用于检查文件的状态以及了解文件的属性。
...
* `-n`：查看前多少行的内容，<mark style="color:$info;">e.g.</mark> <mark style="color:$info;"></mark><mark style="color:$info;">`head -n 200 filename`</mark> <mark style="color:$info;"></mark><mark style="color:$info;">查看</mark> <mark style="color:$info;"></mark><mark style="color:$info;">`filename`</mark> <mark style="color:$info;"></mark><mark style="color:$info;">前</mark> <mark style="color:$info;"></mark><mark style="color:$info;">`200`</mark> <mark style="color:$info;"></mark><mark style="color:$info;">行的内容。</mark>
* `-c`：查看前多少个字节。
...
* `-f`：循环读取文件末尾的内容，一般用来监视输出的日志。<mark style="color:$info;">e.g.</mark> <mark style="color:$info;">`tail -f data.log`</mark><mark style="color:$info;">。</mark>
* `-n`：显示文件的尾部 `n` 行内容，默认显示后 `10` 行。<mark style="color:$info;">e.g.</mark> <mark style="color:$info;">`tail -n 5 notes.log`</mark><mark style="color:$info;">，</mark><mark style="color:$info;">`tail -n +5 notes.log`</mark> <mark style="color:$info;"></mark><mark style="color:$info;">输出</mark> <mark style="color:$info;"></mark><mark style="color:$info;">`notes.log`</mark> <mark style="color:$info;"></mark><mark style="color:$info;">文件的第</mark> <mark style="color:$info;"></mark><mark style="color:$info;">`+5`</mark> <mark style="color:$info;"></mark><mark style="color:$info;">行至文档末尾。</mark>
...
以默认的方式将文本文件的第一列以 ASCII 码的次序排列，并将结果输出到标准输出。

e.g. 使用 `cat` 命令显示 `testfile` 文件可知其原有的排序如下：
...
当重复的行并不相邻时，`uniq` 是不起作用的，即若文件内容为以下时，`uniq` 不起作用：
...
Advanced Copy 是针对 `GNU cp` 和 `GNU mv` 程序的修改版。它添加了一个进度条，并在您复制或移动文件和文件夹时提供一些有关当前进度的信息。它不仅显示进度条，还显示数据传输速率、预计剩余时间以及当前正在复制的文件名。

最后，您将看到一个简短的摘要，显示已复制的文件数量以及复制文件所用的时间。
...
此命令将提取 coreutils 档案到 `coreutils-8.32` 当前目录中名为的文件夹中。
