...
当以 root 身分更改了系统时间之后，请记得以 `clock -w` 来将系统时间写入 **CMOS** 中，这样下次重新开机时系统时间才会持续保持最新的正确值。
...
`-n` 参数，查看前多少行的内容，e.g. `head -n 200 filename` 查看 `filename` 前 `200` 行的内容；

`-c` 参数，查看前多少个字节。

### tail命令 查看文件尾部内容

`-f` 参数，循环读取文件末尾的内容，一般用来监视输出的日志，e.g. `tail -f data.log`

`-n` 参数，显示文件的尾部 `n` 行内容，默认显示后 `10` 行，e.g. `tail -n 5 notes.log`
