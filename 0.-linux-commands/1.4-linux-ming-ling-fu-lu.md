...
[zhangzh@node01 alldata]$ expr 3 % 2
1
[zhangzh@node01 alldata]$ expr 3 \* 2    # 防止 * 被 bash 误判
6
[zhangzh@node01 alldata]$ expr 3 / 2
1
[zhangzh@node01 alldata]$ expr 3. / 2
...
14
[zhangzh@node01 alldata]$ expr substr "this is a test" 3 5  # 统计 3-5 的字符
is is
[zhangzh@node01 alldata]$ expr substr "this is a test" 3 6
is is 
[zhangzh@node01 alldata]$ expr substr "this is a test" 3 7
is is a
[zhangzh@node01 alldata]$ expr index "sarasara"  a        #统计字符 a 出现的位置
2
...
用于指定增量从首数开始打印数字到尾数，全拼为\<Sequence>。产生从某个数到另外一个数之间的所有整数，并且可以对整数的格式、宽度、分割符号进行控制。e.g.
...
-w, --width=WIDTH：设置输出的行宽，默认为 75 个字符。 
```
...
```
选项说明（部分）：
-n：查看前多少行的内容，e.g. head -n 200 filename 查看 filename 前 200 行的内容。
-c：查看前多少个字节。
```
...
```
选项说明（部分）：
-f：循环读取文件末尾的内容，一般用来监视输出的日志。e.g. tail -f data.log。
-n：显示文件的尾部 n 行内容，默认显示后 10 行。e.g. tail -n 5 notes.log，tail -n +5 notes.log 输出 notes.log 文件的第 +5 行至文档末尾。
```
...
如果要停止运行，你需要使用以下命令查找到 nohup 运行脚本到 PID，然后使用 kill命令 来关闭：
...
tree -f：显示文件全路径
tree -L n：n 表示显示项目的层级，n=3 即只显示项目的三层结构 tree -L 2 只显示 2 层
tree -P .pl：只显示文件目录和 .pl 的 perl 文件。
```
...
Hello a 95  a 2005 
Linux a 85  b 2009 
[zhangzh@node01 ~]$ join -1 2 -2 2 1 2    // 按 文件1 和 2 的 2 栏合并
a Hello 95  Hello 2005 
a Linux 85  Hello 2005 
b test 30  Linux 2009 
[zhangzh@node01 ~]$ join -a1 -a2 -1 2 -2 2 1 2    // 输出 文件1，2 的不匹配行
a Hello 95  Hello 2005 
a Linux 85  Hello 2005 
b test 30  Linux 2009 
c tast 2006 
[zhangzh@node01 ~]$ join -e "NU" -a1 -a2 -1 2 -2 2 1 2    // 将 NU 作为空字符填充
