...
shell 顾名思义为「壳」，这是相对于系统内核来讲，可以理解为是内核的「壳」。直接接触内核是不被允许并危险的，所以由 shell 负责将用户的指令解释并传给内核执行，可以把 shell 看作是一种命令解释器。常见的 shell 有很多，分为图形界面 shell 和命令行 shell，命令行中最常用的就是 bash shell。
...
[Vim](https://www.vim.org/)（Vi iMproved）是从 vi 发展出来的一个文本编辑器。具有代码补全、错误跳转等功能，在程序员中被广泛使用。Vim 分为三种模式，分别是**命令模式（Command mode）**，**输入/插入模式（Insert mode）**;**底线命令/命令行模式（Last line mode）**。 这三种模式的作用分别是：
...
按下键盘 `ESC` 键，退出输入模式，切换到命令模式。完成文件的编辑后，在<mark style="color:violet;">命令行模式</mark>中输入 `wq` 存盘退出，也可使用 `Shift-ZZ` 来达到同样的效果。
...
<mark style="color:$danger;">**错误用法 \[0 -eq 1] ；正确用法 \[ 0 -eq 1 ]**</mark>
