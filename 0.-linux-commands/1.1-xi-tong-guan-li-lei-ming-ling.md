---
description: 计算机的英文原词「computer」是指从事数据计算的人。而他们往往又需要借助某些机械计算设备（算盘）或模拟计算机完成工作。
---

# 1.1 系统管理类命令

计算机发明早期体量非常巨大，动辄按吨计算，只能放在专门的机房里。因此，用户使用一种叫「终端」（Terminal Emulator）的外部设备操作计算机。终端只负责输入和显示，不参与计算。后来，随着电子技术和个人计算机的发展，演化出了操作系统，用户不再需要额外的终端设备操作计算机。但终端的概念在计算机操作系统里以「终端模拟器软件」的形式保留了下来。
