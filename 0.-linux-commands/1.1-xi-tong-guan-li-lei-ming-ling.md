---
description: 计算机的英文原词「computer」是指从事数据计算的人。而他们通常又需要借助某些机械计算设备（如算盘）或模拟计算机完成工作。
---

# 1.1 系统管理类命令

计算机发明早期体量非常巨大，动辄按吨计算，只能放在专门的机房里。因此，用户使用一种叫「终端」（Terminal Emulator）的外部设备操作计算机。终端只负责输入和显示，不参与计算。后来，随着电子技术和个人计算机的发展，演化出了操作系统。终端则以「终端模拟器软件」的形式在操作系统中保留了下来。

现在，大部分用户都通过图形化 shell 界面与计算机交互，比如 [macOS](3.-mac-terminal.md)。

更多 shell 介绍和使用，参见 [#unix-shell](1.3-bash-introduce.md#unix-shell "mention")。

本节的终端基础命令介绍以 CentOS Linux 8 为例。
...
在大多数 Linux 发行版中，都使用 `hostname` 命令查看或更改主机名。主机名是网络中的计算机身份标识。<mark style="color:$danger;">修改主机名的操作需要重新登录后才能生效。</mark>修改主机名称方法如下：
...
{% hint style="info" %}
在 Linux 中，用户主要分为两大类：普通用户和超级用户（管理员），后者通常特指 root 用户。root 对系统拥有绝对权力；而普通用户只对个人目录<sup>\[1]</sup>下的文件拥有完全的控制权。
...
**su命令**（switch uesr）用于变更使用者身份。

常见用法如：`su username` 或者 `su -c command username`，其含义分别是切换至用户名为 `username` 的账户和切换至 `username` 的账户后执行命令后退出该账户。也可以手动输入 `exit` 退出当前用户。

直接输入 `su` 命令可以从普通用户切换到 `root`。
...
{% hint style="info" %}
`-p` 选项指定[端口](1.1-xi-tong-guan-li-lei-ming-ling.md#di-zhi-yu-duan-kou)号为 `8022`（不指定默认端口号为 `22`）；`zhangzh` 是登录[账户名](1.1-xi-tong-guan-li-lei-ming-ling.md#yong-hu-guan-li)；`@` 后面是访问[地址](1.1-xi-tong-guan-li-lei-ming-ling.md#di-zhi-yu-duan-kou)。
{% endhint %}

【注】开启 GUI 转发服务时，需要额外添加 `-XY` 选项。在 macOS 中可以[#shi-yong-xquartz-diao-yong-gui-jie-mian](3.-mac-terminal.md#shi-yong-xquartz-diao-yong-gui-jie-mian "mention")。
...
每次登录都输入密码未免太过麻烦，Linux 支持免密登陆。

在值得信任的计算机上执行 `ssh-keygen` 命令，生成 ssh 钥匙。macOS 中钥匙的路径存放在 `.ssh/`。e.g.
...
在终端中上传和下载本地或服务器文件的方法参见[#kua-fu-wu-qi-chuan-wen-jian](1.2-wen-jian-guan-li-lei-ming-ling.md#kua-fu-wu-qi-chuan-wen-jian "mention")。
...
用户管理涉及到<mark style="color:purple;">**组**</mark>与<mark style="color:purple;">**用户**</mark>以及权限的概念。普通用户只对自己的文件具有完整权限，同组用户拥有相对完整的权限，组外（其他）用户不具备任何权限。权限范围可以手动修改，参见[#quan-xian](1.2-wen-jian-guan-li-lei-ming-ling.md#quan-xian "mention")。
...
{% hint style="danger" %}
当创建用户时出现 `Creating mailbox file: File exists` 的错误。可以直接删除 `/var/spool/mail/` 和 `/home` 下的之前存在的用户文件再重建。
{% endhint %}
...
每一个运行中的程序、脚本或后台守护都被称为一个**进程**（Process）。当服务器运行变慢、内存被占满，或者某个分析脚本卡死时，可以根据需要监控并管理这些进程。

**top命令** 查看进程及设备运行情况。

界面支持个性化：按 `Z` 键更改界面颜色，按 `shift+Z`，依次点按 `S` 和数字 `0-7` 可以更改对应位置的字体颜色，按 `T` 和 `M` 键可以切换显示 CPU、GPU 的占用显示，回车后按 `W` 键保存更改。
...
**w命令** 显示登录用户列表、用户正在执行的进程、登录地点、登录时间等信息。

该命令和 `uptime` 一样可以查看设备时间。更多时间命令可以参考 [1.4-linux-ming-ling-fu-lu.md](1.4-linux-ming-ling-fu-lu.md "mention")。w命令部分选项如下：
...
<mark style="color:$success;">**jobs命令**</mark> <mark style="color:$success;">显示作业的状态，列出活动和停止的作业。</mark>

`jobs` 常用选项是 `jobs -l`：以包含 `pid` 的形式展示进程。

【注】`jobs` 只能查看当前账户的进程。诸如 `nohup` 之类的命令，重新登录后 `jobs` 就无法查询，这时候可以使用 `ps` 命令来管理。

<mark style="color:$success;">**ps命令**</mark><mark style="color:$success;">（process status）报告当前系统的进程状态。</mark>

通常搭配 `kill` 指令随时中断、删除不必要的程序。`ps` 是最基本和的进程查看命令。如 `ps -aux` 配合 `grep` 命令查找需要的进程，并打印 ID 号和所属用户。

**pstree命令** 以一种优雅的方式展示进程树。

#### 当进程严重影响到服务器的运行时，管理员可以结束特定用户的特定进程。

**kill命令** 通过进程 ID（Process ID, PID）关闭进程。

<mark style="color:$success;">使用</mark> <mark style="color:$success;">`kill -9`</mark> <mark style="color:$success;">命令立即杀死指定 PID 的进程。</mark>使用 `ps` 等命令配合 `grep` 来查找 PID。

**killall命令** 通过**指定用户名**或**进程名称**的方式关闭进程。

**pkill命令** `pkill` 和 `killall` 应用方法差不多，但支持正则表达。如果想杀掉单个进程，使用 `kill`。
...
在变量名前的 `$` 不仅能显示变量当前的值，还能让变量作为命令行参数。e.g.
...
用户从 GitHub 或官网下载的开源软件通常只是**源代码**。因此必须使用编译器将文本代码转化为可执行程序，这个过程就叫做**编译**。
...
<mark style="color:$warning;">每次配置别名仅在当前 bash 中生效，</mark>别名配置保存在 `~/.bashrc` 中将全局生效，详情参考这篇[文章](http://superuser.com/a/183980/7106)。
