...
早期的计算机非常巨大，动辄按吨计算，只能放在专门的机房里。为了操作这些机器，人们使用一种叫「终端」的外部设备（Terminal Emulator）。终端只负责输入和显示，不负责计算。后来个人计算机兴起，操作都在一台机器上完成，不再需要额外的终端设备。不过，终端的概念在操作系统里保留了下来，变成软件形式的「终端模拟器」，大家还是叫它终端。

现在，我们大多通过图形化 shell 界面与计算机交互，比如 [macOS](3.-mac-terminal.md) 就是这样。

更多关于 shell 介绍和使用，参见 [#unix-shell](1.-bash-introduce.md#unix-shell "mention")。

本节的基础命令将以 CentOS Linux 8 为例。
...
在大多数 Linux 发行版中，都使用 `hostname` 命令查看或更改主机名。主机名用于网络中的计算机标识。<mark style="color:$danger;">**修改主机名操作需要重新登陆后才能生效。**</mark>修改主机名称方法如下：
