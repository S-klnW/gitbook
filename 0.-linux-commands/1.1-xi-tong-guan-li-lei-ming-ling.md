...
{% hint style="info" %}
`-p` 选项表示指定端口号；`8022` 是指定的[端口](1.1-xi-tong-guan-li-lei-ming-ling.md#di-zhi-yu-duan-kou)（不指定端口默端口号为 `22`）；`zhangzh` 是登录账户名；`@` 后面是访问[地址](1.1-xi-tong-guan-li-lei-ming-ling.md#di-zhi-yu-duan-kou) `222.2.2.102`。
{% endhint %}
...
在终端中上传和下载本地或服务器文件的方法参见[#kua-fu-wu-qi-chuan-wen-jian](1.2-wen-jian-guan-li-lei-ming-ling.md#kua-fu-wu-qi-chuan-wen-jian "mention")章节内容。
...
用户管理涉及到<mark style="color:purple;">**组**</mark>与<mark style="color:purple;">**用户**</mark>以及权限的概念。通常，普通用户只对自己的文件具有完整权限，同组用户拥有相对完整的权限，对组外（其他）用户不具备任何权限。权限控制介绍参见[#quan-xian](1.2-wen-jian-guan-li-lei-ming-ling.md#quan-xian "mention")。
...
**w命令** 显示登录用户列表、用户正在执行的进程、登录地点、登录时间等信息。还可用于查看系统时间，这一点和 `uptime` 作用一致。更多关于时间的命令可以参考 [1.4-linux-ming-ling-fu-lu.md](1.4-linux-ming-ling-fu-lu.md "mention")。该命令参数如下：
