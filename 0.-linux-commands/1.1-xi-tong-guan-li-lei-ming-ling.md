...
示例代码中 `[zhangzh@node01 ~]$` 分别表示以用户名为 `zhangzh` 的用户登录主机名为 `node01` 的主机。`su` 命令可以从普通用户切换到 `root shell` 中，输入 `exit` 可以退出 `root shell`。
...
{% hint style="info" %}
`-p` 选项表示指定端口号；`8022` 是指定的端口（不指定端口默端口号为 `22`）；`zhangzh` 是登录账户名；`@` 后面是访问地址 `222.2.2.102`。
{% endhint %}
