...
**ssh命令** 用来远程连接服务器，启动 X11 服务时（调用 GUI）需要添加`-XY` 参数。_**E.g.**_
...
不同系统的服务启动指令略有差异，本系统使用`systemctl start git`启动。_**E.g.**_
...
还可用于查看系统时间，这一点和`uptime`作用一致。_**E.g.**_
...
`systemctl status` 是一个用于在 Linux 系统上查看服务状态的命令。它可以显示特定服务的详细信息，包括当前是否正在运行、最后一次启动时间、PID、主机名称等。_**E.g.**_
...
<summary><em><strong>E.g. 通配符使用例子</strong></em></summary>
