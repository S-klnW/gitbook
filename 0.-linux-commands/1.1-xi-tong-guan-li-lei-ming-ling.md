...
Linux本身指内核，只有加上其他一些组件之后才是真正意义上的“系统”。后来随着Linux的流行，人们逐渐默认了以Linux代指系统，严格的讲应该是GNU/Linux系统。
...
打开个人电脑的终端之后，就是打开了一个shell界面，我们通过shell和计算机进行交互。本节将整理介绍一些基础的系统管理类命令-以CentOS 8为例。
...
用`ssh`命令来远程连接服务器，如需启动X11服务（即调用GUI），请添加`-XY` 参数，_**E.g.**_
