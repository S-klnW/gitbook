...
其次掌握 **type命令，**判断命令是可执行文件、shell内置命令还是别名，`type`的使用方法参考[附录](1.4-linux-ming-ling-fu-lu.md)。
...
<mark style="color:red;">不要把所有的文件都存放在主目录中。通过创建子目录来更有效地整理文件，最好采用前后一致的命名方式来区分文件和目录。</mark> **E.g.**
...
<summary><strong>E.g. ln命令的使用</strong></summary>
...
<summary><strong>E.g. 不同参数的 du命令的统计效果</strong></summary>
