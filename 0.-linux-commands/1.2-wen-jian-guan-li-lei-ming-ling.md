...
* `[ ]`（方括号）：代表可选参数。例如 `[--color]`，意味用户是否使用这个参数，命令都能运行。
* `< >`（尖括号）：代表必填参数。用户必须在后面传入具体的文件名、路径或数值。
* `|`（竖线）：代表互斥（多选一）。例如 `a|b` 意味着用户只能在 `-a` 和 `-b` 参数中选择一个，不能同时使用。
* `...`（省略号）：代表可以同时传入多个同类参数。例如 `ls [FILE]...` 意味着用户可以一次性查看多个文件。
* 按 `q` 键退出手册，返回终端。
...
**df命令**（disk free）用于<mark style="color:purple;">显示文件系统的磁盘空间利用情况</mark>。当在终端中运行 `df` 命令时，它会列出系统中所有的文件系统及其对应的磁盘空间使用情况，包括磁盘总容量、已用空间、可用空间以及挂载点等信息。这个命令对于了解系统中磁盘空间的使用情况非常有用。建议使用 `-h` 选项使之易读。
...
**wc命令** 可以计算文件的 Byte 数、字数（使用 `-w` 选项)、或是行数（使用 `-l` 选项）。
...
**mv命令（move file）** 用来对文件或目录重新命名，或者将文件从一个目录移到另一个目录中。如果将一个文件移到一个已经存在的目标文件中，则目标文件的内容将被覆盖。
