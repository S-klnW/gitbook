...
**type命令** 判断命令是可执行文件、shell 内置命令还是别名，`type` 的使用方法参考[#type-ming-ling-xian-shi-zhi-ding-ming-ling-de-lei-xing](1.4-linux-ming-ling-fu-lu.md#type-ming-ling-xian-shi-zhi-ding-ming-ling-de-lei-xing "mention")。
...
{% hint style="info" %}
`ll`是`ls -a`的别名，别名的配置方法参见[#pei-zhi-bie-ming](1.1-xi-tong-guan-li-lei-ming-ling.md#pei-zhi-bie-ming "mention")。
{% endhint %}
...
{% hint style="success" %}
软链接，是将一个路径名链接到一个文件。这些文件是一种特别类型的文件。事实上，它只是一个文本文件，其中包含它提供链接的另一个文件的路径名，如图中虚线箭头所示。另一个文件是实际包含所有数据的文件。所有读、写文件内容的命令被用于符号链接时，将沿着链接方向前进来访问实际的文件。<mark style="color:$success;">所以建议使用软连接</mark><mark style="color:$success;">。</mark>
{% endhint %}
...
删除目录可以使用 `rm -rf` 命令或者 **rmdir命令**（Remove directory）。
