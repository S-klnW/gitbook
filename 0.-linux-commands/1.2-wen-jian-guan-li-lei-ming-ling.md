---
description: >-
  小行星 9793 是一颗围绕太阳公转的小行星。1996 年 1 月 16 日，太空监视在基特峰发现了此天体。该天体以 Linux 操作系统的创建者
  Linus Torvalds 命名。
---
...
* `[ ]`（方括号）：代表可选选项。例如 `[--color]`，意味用户是否使用这个选项，命令都能运行。
* `< >`（尖括号）：代表必填选项。用户必须在后面传入具体的文件名、路径或数值。
* `|`（竖线）：代表互斥（多选一）。例如 `a|b` 意味着用户只能在 `-a` 和 `-b` 选项中选择一个。
* `...`（省略号）：代表可以同时传入多个同类选项。例如 `ls [FILE]...` 意味着用户可以一次性查看多个文件。
* 按 `q` 键退出手册。

**type命令** 判断命令是可执行文件、shell 内置命令还是别名，`type` 的使用方法参考 [#type-ming-ling-xian-shi-ming-ling-lei-xing](1.4-linux-ming-ling-fu-lu.md#type-ming-ling-xian-shi-ming-ling-lei-xing "mention")。
...
* <mark style="color:$success;">创建新的空文件。</mark>
...
{% hint style="info" %}
`ll` 是 `ls -l` 的别名，别名的配置方法参见 [#pei-zhi-bie-ming](1.1-xi-tong-guan-li-lei-ming-ling.md#pei-zhi-bie-ming "mention")。

【附】创建 `lab` 路径后，[权限](1.2-wen-jian-guan-li-lei-ming-ling.md#quan-xian) `rwx------` 前的字母由 `-` 变成 `d`，表明这是一个路径。
{% endhint %}
...
**zip命令** 用来压缩和解压缩文件。文件经它压缩后会另外产生具有 `.zip` 扩展名的压缩文件。
...
Linux 具有为一个文件创建多个访问入口的功能，称为链接。被链接的文件可以存放在相同的目录下，但是必须有不同的文件名，而不用在硬盘上为同样的数据重复备份。
...
{% hint style="success" %}
软链接，是将一个路径名链接到一个文件。这些文件是一种特别类型的文件。事实上，它只是一个文本文件，其中包含它提供链接的另一个文件的路径名。另一个文件是实际包含所有数据的文件。所有读、写文件内容的命令被用于符号链接时，将沿着链接方向前进来访问实际的文件。<mark style="color:$success;">所以建议使用软连接。</mark>
{% endhint %}
...
**df命令**（disk free）显示文件系统的磁盘空间利用情况。

当在终端中运行 `df` 命令时，它会列出系统中所有的文件系统及其对应的磁盘空间使用情况，包括磁盘总容量、已用空间、可用空间以及挂载点等信息。这个命令对于了解系统中磁盘空间的使用情况非常有用。建议使用 `-h` 选项使之易读。
...
**du命令**（disk usage）查看文件和目录磁盘的使用空间。

<mark style="color:$success;">常用命令是</mark> <mark style="color:$success;">`du -sh`</mark> <mark style="color:$success;">和</mark> <mark style="color:$success;">`du -h`</mark><mark style="color:$success;">，前者可以查看当前目录下所有文件大小的总和，后者可以列出当前目录下所有文件的大小。</mark>

<details>

<summary>e.g. 常用选项和实例</summary>
...
**wc命令** 计算文件的 Byte 数、字数（使用 `-w` 选项)、或是行数（使用 `-l` 选项）。



## 复制/移动/重命名/删除

**cp命令（copy file）** 将一个或多个文件或者目录复制到指定的文件或目录中。

<mark style="color:$danger;">**当一次复制多个文件时，目标文件参数必须是一个已经存在的目录，否则将出现错误。**</mark>

**mv命令（move file）** 对文件或目录重命名，或者将文件从一个目录移到另一个目录中。
...
**语法：**`source` 表示源文件或目录，`target` 表示目标文件或目录。
...
在两台服务器之间传输文件可以使用 **`scp` 命令**，它的地址格式与 `ssh` 基本相同，稍微的差别是指定端口时选项 `-P` 是大写。**`scp` 与 `ssh`** 一样支持「别名」。
