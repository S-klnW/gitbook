...
```shell
root [12] TBrowser t    // 同样可以通过->StartViewer()指令打开图形界面 
...
> 常用于若干ROOT文件的合并，并不推荐大量root文件的合并
...
![输入图片说明](.gitbook/assets/chain.svg)

### hadd

> hadd命令常用于合并文件，但通常在大文件下，不建议使用该命令处理，建议生成列表后遍历
