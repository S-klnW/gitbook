...
<mark style="color:red;">0.68</mark>

写入bin内容 histo->SetBinContent(i+1, data\[i]); histo->SetBinError(i+1, TMath::Sqrt(data\[i])), bin从1开始

坐标轴的中心bin&#x20;

Double\_t xcenter = h1->GetXaxis()->GetBinCenter(i)&#x20;

坐标轴的最大值&#x20;

gPad->GetUymax(), 在此之前需要运行c1->Update()
